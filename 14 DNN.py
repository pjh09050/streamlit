...
############################################################################
with st.sidebar:
    choose = option_menu("순서", ["데이터 선택", "모델 선택", "데이터 학습", "성능 확인"], icons=['bookmark-heart','bookmark-heart','bookmark-heart','bookmark-heart'], menu_icon="bi bi-card-list",
        styles={
        ...
        if model_select == 'Deep Neural Network':
            ...
            if train_start == True:
                ...
                if pred_check == True:
                    ...
                    if data_trans == True:
                        st.session_state['y_pred_dnn'] = y_pred
                        st.session_state['y_test_dnn'] = y_test
